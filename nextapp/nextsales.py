...
def get_customer_sales(query='',last_day=0, sort='',page=0):
	...
	for d in data:
		fetchTotalSales  = frappe.db.sql("SELECT ISNULL(SUM(rounded_total),0) FROM `tabSales Invoice` WHERE customer_name = '{}' AND posting_date BETWEEN DATE(NOW()) - INTERVAL {} DAY AND NOW()".format(d["customer_name"],last_day))
