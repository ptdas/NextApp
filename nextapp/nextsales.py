...
def get_customer_sales(query='',last_day=0, sort='',page=0):
	...
	for d in data:
		...
		fetchTotalSales  = frappe.db.sql("SELECT COALESCE(SUM(rounded_total * conversion_rate),0) FROM `tabSales Invoice` WHERE docstatus = 1 AND customer = '{}' AND posting_date BETWEEN DATE(STR_TO_DATE('{}','%Y-%m-%d %H:%i:%s') - INTERVAL {} DAY) AND STR_TO_DATE('{}','%Y-%m-%d %H:%i:%s')".format(escape_string(d['name']),now,last_day,now))
